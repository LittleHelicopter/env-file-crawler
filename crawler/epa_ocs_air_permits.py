...
仅抓取class="l-sidebar__main"部分的链接
仅抓取两层，（进入第二层后只抓取pdf）
"""
...
from utils.web_utils import remove_header_footer, get_links_from_specific_div_dynamic
...
# 配置日志记录
logging.basicConfig(filename="../logs/outer-continental-shelf-air-permits.txt",  # 日志文件名
...
start_urls = ["https://www.epa.gov/caa-permitting/outer-continental-shelf-air-permits"]

# 为每个 URL 生成简短的文件名
url_to_filename = {
    "https://www.epa.gov/caa-permitting/outer-continental-shelf-air-permits": "epa_ocs_air_permits"
}
...
def process_first_and_second_layer(base_url, pdf_folder, other_files_folder):
    ...
    try:
        # 获取当前页面的链接
        # start链接页面上的链接
        links = get_links_from_specific_div_dynamic(base_url, div_class="l-sidebar__main")
...
def process_second_layer(url, pdf_folder, other_files_folder):
    ...
    try:
        # 获取当前页面的链接
        pdf_links = get_links_from_specific_div_dynamic(url)
